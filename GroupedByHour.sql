...
SELECT DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0) [GroupValue]
...
, DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0) [GroupByHours]
...
GROUP BY
 DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0), m.[SiteID]
ORDER BY 
	DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset),0), m.[SiteID] DESC, [Value] DESC
