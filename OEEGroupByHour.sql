...
SELECT 
 ROW_NUMBER() OVER (ORDER BY a.DisplayOrder, DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0)) [rownumber]
, DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0) [GroupValue]
, DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0) [GroupLabel]
, DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0) [GroupDisplayOrder]
, a.Name [SeriesValue] 
, a.Name [SeriesLabel] 
, a.DisplayOrder [SeriesDisplayOrder] 
...
GROUP BY
DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0), a.Name, a.DisplayOrder
ORDER BY a.DisplayOrder,DATEADD(hour, DATEDIFF(hour, 0, m.StartDateTimeOffset), 0)