...
SELECT area.[Name] + ' ' + m.[EventDefinitionName] [GroupValue]
, m.[EventDefinitionName] [GroupLabel]
, m.[EventDefinitionDisplayOrder] [GroupDisplayOrder]
, 1 [SeriesValue] 
, area.Name [SeriesLabel] 
...
FROM dbo.[viewEvent] m
 ...
 INNER JOIN tArea area ON area.ID = j.AreaID
WHERE 
...
GROUP BY
 area.[Name], AreaID, area.AltName, m.[EventDefinitionName], ed.Color, m.[EventDefinitionDisplayOrder], m.[SiteID]
ORDER BY CAST(area.AltName as int), m.[EventDefinitionName], m.[SiteID] DESC, [Value] DESC
